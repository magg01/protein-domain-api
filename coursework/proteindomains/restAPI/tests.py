...
class PfamTest(APITestCase):
    ...
    def test_PfamDetailDomainDescriptionRecievedCorrect(self):
        response = self.client.get(self.good_url)
        response_json = json.loads(response.content)
        self.assertEqual(response_json['domain_description'], "Kunitz/Bovinepancreatictrypsininhibitordomain")
...
class ProteinDetailSerializer(APITestCase):
    protein1 = None
    proteinDetailSerializer = None
    data = None

    def setUp(self):
        self.protein1 = ProteinFactory.create()
        self.proteinDetailSerializer = ProteinDetailSerializer(instance=self.protein1)
        self.data = self.proteinDetailSerializer.data
    ...
    def test_proteinDetailSerializer(self):
        self.assertEqual(set(self.data.keys()), set(['protein_id', 'sequence','taxonomy','length','domains']))

    def test_proteinDetailSerializerProteinIdHasCorrectData(self):
        self.assertEqual(self.data['protein_id'], self.protein1.protein_id)

    def test_proteinDetailSerializerSequenceHasCorrectData(self):
        self.assertEqual(self.data['sequence'], self.protein1.sequence)